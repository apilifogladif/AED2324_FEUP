...
bool Graph<T>::addEdge(const T &sourc, const T &dest, double w) {
    ...
    auto sourceVertex  = findVertex(sourc);
    if (sourceVertex  == NULL) return false;
    auto destVertex  = findVertex(dest);
    if (destVertex  == NULL) return false;
...
bool Graph<T>::removeVertex(const T &in) {
    ...
    for (Vertex<T>* vertex : this->vertexSet) {
        vertex->removeEdgeTo(v);
    }
    auto it = std::find(this->vertexSet.begin(), this->vertexSet.end(), v);
